(function(){
  ...
  ochishTugma.addEventListener('click', function(){
    ...
    if (ochiq) sozlamalarBlok.scrollIntoView({behavior:'smooth', block:'center'});
  });
  ...
  var profilTugma = el('profilTugma');
  var authModalFon = el('authModalFon');
  var authKirishBlok = el('authKirish'), authRoyxatBlok = el('authRoyxat'), authProfilBlok = el('authProfil');

  function authOch(){
    authModalFon.classList.add('ochiq');
    document.body.style.overflow = 'hidden';
  }
  function authYop(){
    authModalFon.classList.remove('ochiq');
    document.body.style.overflow = '';
  }
  profilTugma.addEventListener('click', authOch);
  el('authModalYop').addEventListener('click', authYop);
  authModalFon.addEventListener('click', function(ev){ if (ev.target === authModalFon) authYop(); });
  document.addEventListener('keydown', function(ev){
    if (ev.key === 'Escape' && authModalFon.classList.contains('ochiq')) authYop();
  });

  el('authRoyxatgaOt').addEventListener('click', function(){
    authKirishBlok.hidden = true; authRoyxatBlok.hidden = false;
  });
  el('authKirishgaOt').addEventListener('click', function(){
    authRoyxatBlok.hidden = true; authKirishBlok.hidden = false;
  });
  ...
  function profilKorsat(user){
    ...
    profilTugma.innerHTML = '<span class="profil-doira">' + nom.charAt(0).toUpperCase() + '</span>';
  }
  ...
  if (!FIREBASE_TAYYOR){
    ...
    ['authKirishTugma','googleKirishTugma1','googleKirishTugma2'].forEach(function(id){
      el(id).addEventListener('click', function(){ authXabarKorsat('authXabar', M('auth_hali_yoq'), true); });
    });
    el('authRoyxatTugma').addEventListener('click', function(){ authXabarKorsat('authXabar2', M('auth_hali_yoq'), true); });
    el('authParolUnut').addEventListener('click', function(){ authXabarKorsat('authXabar', M('auth_hali_yoq'), true); });
  } else {
    ...
    firebase.auth().onAuthStateChanged(function(user){
      ...
      if (!user){
        ...
        profilTugma.textContent = '\U0001F464';
        boshPanelChiz();
        kundalikChiz();
        return;
      }
